...
# Install CLIP if not present: pip install git+https://github.com/openai/CLIP.git
try:
    import clip
except ImportError:
...
# ==========================================
# 1. MODELS (CLIP Vision + Cnn14 Audio)
# ==========================================

class VisionEncoder(nn.Module):
    def __init__(self, embedding_dim=128):
        super().__init__()
        # Load CLIP model (downloading typically happens once)
        print("Loading CLIP (ViT-B/32)...")
        self.clip_model, _ = clip.load("ViT-B/32", device=DEVICE)
        
        # 1. Freeze CLIP Backbone completely
        ...
        # Convert model to float32 (CLIP often loads in float16)
        self.clip_model = self.clip_model.float()

    def forward(self, x):
        # CLIP's encode_image expects float32 tensors
        with torch.no_grad():
...
# ==========================================
# CLIP has specific normalization requirements
...
def compute_metrics(model, v_dir, a_dir, k_values=[1, 5, 10]):
    ...
    a_labels = np.array(final_a_labels)
...
def main():
    ...
    best_avg_r5 = 0.0
    print(f"--- Starting CLIP (Vision) + Cnn14 (Audio) Training ---")
    ...
    for epoch in range(20):
        ...
        if avg_r5 > best_avg_r5:
            ...
            print("  🔥 Best CLIP-Cnn14 Model Saved!")
