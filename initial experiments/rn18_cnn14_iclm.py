...
# --- C. Instruction-Based Cross-Learning Module (ICLM) ---
class ICLM(nn.Module):
    ...
    def forward(self, v_o, a_o):
        ...
        # Eq 21: V_Ai (Visual instructed by Audio)
        ...
        # Eq 22: A_Vi (Audio instructed by Visual)
...
def compute_metrics(model, v_dir, a_dir, k_values=[1, 5, 10], batch_size=64):
    ...
    a_loader = DataLoader(a_ds, batch_size=batch_size)
    ...
    with torch.no_grad():
        for i in tqdm(range(N), leave=False):
            ...
            # Chunking the M audios to avoid VRAM OOM during interaction
...
def main():
    ...
    # Use the Cosine Triplet Loss aligned with the paper's hinge objective
    ...
    print(f"--- Training with ICLM & Cosine Hinge Loss ---")
    ...
    for epoch in range(max_epochs):
        ...
        for img_anc, img_neg, aud_pos, aud_neg in pbar:
            ...
            v_f_anc_neg, a_f_neg_a = model.iclm(v_o_anc, a_o_neg) # Image Anchor vs Audio Neg
            v_f_neg_v, a_f_pos_neg = model.iclm(v_o_neg, a_o_pos) # Image Neg vs Audio Pos
