...
def get_args():
    ...
    # Ensure this points to your trained SARCI model weights
...
def main():
    ...
    target_classes = all_classes # PROCESS EVERY CLASS
    ...
    # FIXED SEED: Guarantees exact same random audio is picked across model scripts!
    random.seed(100) 
    
    for cls in target_classes:
        a_files = sorted(glob.glob(os.path.join(VAL_A, cls, "*.wav")))
        if a_files:
            wav_path = random.choice(a_files) # Randomly pick one deterministically
    ...
    with torch.no_grad():
        for i in tqdm(range(0, len(gallery_paths), batch_size), desc="Extracting Image Gallery"):
            ...
            # SARCI base features (before ICLM interaction)
    ...
    # ----------------------------------------
    # E. RETRIEVAL & PLOTTING (WITH ICLM)
    # ----------------------------------------
    for q_cls, wav_tensor in queries.items():
        ...
        plt.tight_layout()
        save_name = os.path.join(args.output_dir, f"SARCI_query_{q_cls}.png")
