...
from PIL import Image  # Fixed the missing import here
...
def get_args():
    ...
    # Defaulting to the attention model weights
...
# ==========================================
# 1. ATTENTION MODULES (Must Match Training)
# ==========================================
...
# ==========================================
# 2. ENCODERS (Must Match Training)
# ==========================================
class VisionEncoder(nn.Module):
    def __init__(self, embedding_dim=128):
        super().__init__()
        resnet = resnet18(weights=None) # Structure only
...
class AudioEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        return F.normalize(self.project(torch.cat([mu, std], 1)), p=2, dim=1)
...
def plot_joint_tsne(v_feats, a_feats, v_labels, a_labels, class_names):
    ...
    plt.title("Joint t-SNE (Attention): Vision (▲) vs Audio (●)")
    plt.tight_layout()
    plt.savefig("attention_tsne_joint.png")
    print("Saved attention_tsne_joint.png")
    return reduced 
...
def plot_class_subplots(reduced_all, v_labels, a_labels, class_names):
    ...
    plt.tight_layout()
    plt.savefig("attention_tsne_subplots.png", dpi=300)
    print("Saved attention_tsne_subplots.png")
...
def main():
    ...
    print("\n" + "="*40)
    print("   FINAL RETRIEVAL RESULTS (ATTENTION MODEL)")
    print("="*40)
    ...
    plot_single_confusion(
        ...
        class_names=classes, 
        title="Image-to-Audio (Attention Model)", 
        filename="attention_confusion_img2aud.png",
    ...
    plot_single_confusion(
        ...
        class_names=classes, 
        title="Audio-to-Image (Attention Model)", 
        filename="attention_confusion_aud2img.png",
