...
def get_args():
    ...
    # Defaulting to the finetuned model, change if using the frozen one
    parser.add_argument("--model_path", type=str, default="best_cross_modal.pth")
...
def plot_joint_tsne(v_feats, a_feats, v_labels, a_labels, class_names):
    ...
    plt.title("Joint t-SNE: Vision (▲) vs Audio (●)")
    plt.tight_layout()
    plt.savefig("cross_modal_tsne_joint.png")
    print("Saved cross_modal_tsne_joint.png")
    return reduced 
...
def plot_class_subplots(reduced_all, v_labels, a_labels, class_names):
    ...
    plt.tight_layout()
    plt.savefig("cross_modal_tsne_subplots.png", dpi=300)
    print("Saved cross_modal_tsne_subplots.png")
...
def plot_single_confusion(query_feats, gallery_feats, query_labels, gallery_labels, class_names, title, filename, ylabel, xlabel, cmap='Purples'):
    ...
    for i in range(len(query_labels)):
        # Find the single best match in the gallery
    ...
    # Row-Normalize (so diagonal is Recall@1)
